...
// Marcus Cheecham
// Febuary 13th, 2025
// State Variable, Switch, Frames/Time  
...
function setup() {
  ...
  startTime = millis(); // Should roughly be equal to 0
}
...
function keyPressed() {
  //on each key press, let's advance the state
  //variable 1 → 2 → 3 → 4(for 2 seconds)→ 1
  if (shapeState < 4) {
    shapeState++; 
    if (shapeState === 4) {
      startTime= millis();
...
function draw() {
  ...
  drawShape();
  manageTimer();
  if (shapeState === 4 && elapsedTime > 500) {
    shapeState = 1;
  }
}
